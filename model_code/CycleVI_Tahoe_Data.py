...
class DecoderCCVI(nn.Module):
    """
    Decoder module for CCVI model. Applies Fourier-based modulation for cell cycle–regulated genes
    and a feedforward network for the remaining signal. Supports disabling cell cycle effect.

    Args:
        n_input (int): Total latent dimension (z_cycle + z_latent).
        n_output (int): Number of output genes.
        n_layers (int): Number of hidden layers in the non-cycle network.
        n_hidden (int): Hidden layer width.
        n_cat_list (list[int]): Ignored (included for compatibility).
        inject_covariates (bool): Ignored (included for compatibility).
        use_batch_norm (bool): Use batch norm in non-cycle FC layers.
        use_layer_norm (bool): Use layer norm in non-cycle FC layers.
        scale_activation (str): 'softmax' or 'softplus' for output activation.
        cycle_gene_mask (torch.Tensor): Mask for which genes are regulated by the cell cycle.
        n_fourier (int): Number of harmonics in the Fourier basis.
    """
    def __init__(
        self,
        n_input: int,
        ...
        **kwargs
        ...
        # Feedforward decoder for non-cycle-dependent expression
        self.non_cycle_fc = FCLayers(
            n_in=n_input - 1,  # exclude z_cycle (2D), include drug concentration (1D)
            n_out=n_hidden,
            n_cont=0,
            n_cat_list=None,
            n_layers=n_layers,
        ...
        # Fourier weights (learned only for cycle genes)
        self.fourier_W = nn.Parameter(0.01 * torch.randn(2 * n_fourier, n_output))

        # Mask gradients for non-cycle genes
        ...
        # Gene-wise dispersion (used in NB likelihood)
        self.disp_raw = nn.Parameter(0.01 * torch.randn(n_output))

        # Output activation for gene expression proportions
    ...
    def forward(
        ...
        Args:
            z (Tensor): Latent representation [N, latent_dim], with z_cycle = z[..., :2].
            library (Tensor): Log library size [N, 1].
            d (Tensor): Drug concentration [N, 1].
            t (Tensor): Unused, reserved for future covariates.
            remove_cell_cycle (bool): Whether to zero out cell cycle effect.

        Returns:
            Tuple of outputs (px_scale, dispersion, rate, ..., angle, ..., fourier_W, ..., raw_non_cycle_out)
        """
        assert d.ndim == 2 and d.size(1) == 1, "Expected drug concentration shape [N, 1]"

        z_cycle = z[..., 0:2]         # 2D cell cycle latent
        z_latent = z[..., 2:]         # remaining latent dimensions

        if d is None:
            raise ValueError("Drug concentration `d` must be provided.")

        if z_latent.ndim == 3 and d.ndim == 2:
            d = d.unsqueeze(0).expand(z_latent.shape[0], -1, -1)  # Broadcast for batch mode

        # Decode non-cycle component
        x_input = torch.cat([z_latent, d], dim=-1)
        x = self.non_cycle_fc(x_input)
        non_cycle_out = self.non_cycle_linear(x)

        # Compute cell cycle phase angle
        x, y = z_cycle[..., 0], z_cycle[..., 1]
        angle = torch.atan2(y, x)  # range: [-π, π]

        # Decode cycle component (Fourier series)
        if remove_cell_cycle:
            ...
            cycle_effect = torch.matmul(fourier_basis, self.fourier_W) * self.cycle_mask  # mask non-cycle genes

        # Combine both components
        eta = non_cycle_out + cycle_effect

        # Convert to gene expression rates
        px_scale = self.px_scale_activation(eta)             # normalized expression (proportions)
        px_rate = torch.exp(library) * px_scale              # expected counts
        disp = self.disp_raw                                  # gene-wise dispersion

        return (
            px_scale,            # Softmax or softplus-normalized output
            disp,                # Dispersion parameters
            px_rate,             # Final NB rate
            None,                # Placeholder for optional return
            angle.unsqueeze(1),  # Cell cycle phase angle
            None, None,          # Placeholders
            self.fourier_W,      # Fourier weights for cycle effect
            None,                # Placeholder
            non_cycle_out        # Raw decoder output (before cycle addition)
        )
...
class CC_VAE(EmbeddingModuleMixin, BaseMinifiedModeModuleClass):
    """
    Cell Cycle–aware Variational Autoencoder (CC-VAE) for single-cell RNA-seq.

    This VAE model supports:
    - Disentangled cell cycle modeling via 2D circular latent space.
    - Fourier-based decoding for periodic gene expression.
    - Standard VAE functionality from scvi-tools (e.g., dropout, dispersion modeling, batch correction).
    - Optional angle alignment and radius regularization.

    Inherits from:
    - EmbeddingModuleMixin: provides support for batch/covariate embeddings.
    - BaseMinifiedModeModuleClass: supports full/minified AnnData inputs.
    """
    ...
    def __init__(
        ...
        # Register log library size stats if not using observed
        ...
        # Batch representation: one-hot or embedding
        self.batch_representation = batch_representation
        if batch_representation == "embedding":
            self.init_embedding(REGISTRY_KEYS.BATCH_KEY, n_batch, **(batch_embedding_kwargs or {}))
            batch_dim = self.get_embedding(REGISTRY_KEYS.BATCH_KEY).embedding_dim
        elif batch_representation != "one-hot":
            raise ValueError("`batch_representation` must be 'one-hot' or 'embedding'.")

        # Determine where to apply normalization
        ...
        # Construct encoder input dimension
        n_input_encoder = n_input + (n_continuous_cov if encode_covariates else 0)
        if batch_representation == "embedding":
            n_input_encoder += (batch_dim if encode_covariates else 0)

        cat_list = [n_batch] + (n_cats_per_cov or [])
        ...
        n_input_decoder = n_latent + (batch_dim if batch_representation == "embedding" else 0)
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        px_scale, disp, px_rate, _, angle, radius, _, W_fourier, _, baseline = self.decoder(
            decoder_input, size_factor, d, t, remove_cell_cycle=remove_cell_cycle
        )
...
# Load AnnData object with full path (on cluster)
adata = sc.read_h5ad("adata_HT29_Dapagliflozin_balanced_cycle_angle.h5ad")

# Preserve raw counts
adata.layers["counts"] = adata.X.copy()
adata.raw = adata

# Register fields for scvi-tools
CCVI.setup_anndata(
    adata,
    layer="counts",
    labels_key="cell_name",
    continuous_covariate_keys=["drugconc", "cycle_angle_uniform"],
...
def create_cell_cycle_gene_mask(
    ...
    """
    Create a boolean mask over genes in `adata` indicating cell cycle membership.

    Args:
        adata: AnnData object.
        gene_list_txt: Path to .txt file with one gene name per line.
        var_column: Column in adata.var to match genes. If None, uses adata.var_names.

    Returns:
        torch.BoolTensor of shape (n_genes,)
    """
...
# Generate mask
cycle_gene_mask = create_cell_cycle_gene_mask(adata, "GO_cell_cycle_annotation_human.txt")
...
model = CCVI(
    adata,
    n_latent=10,
    cycle_gene_mask=cycle_gene_mask  # Apply mask here
)
...
model.save("tahoe_model", overwrite=True)