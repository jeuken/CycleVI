    ...
    def __init__(
        ...
        **loss_kwargs,
        ...
        self.adversarial_classifier = Classifier(
            n_input=self.module.n_latent - 2,  # Assuming z_cycle is 2D
            n_hidden=32,
            n_labels=n_phases,
            n_layers=2,
            logits=True,
...
class DecoderCCVI(nn.Module):
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor, remove_cell_cycle: bool = False):
        assert d.ndim == 2 and d.size(1) == 1

        z_cycle = z[..., 0:2]         # works for both 2D and 3D
        z_latent = z[..., 2:]

        if d is None:
            raise ValueError("Drug concentration d must be provided")
        if z_latent.ndim == 3 and d.ndim == 2:
            d = d.unsqueeze(0).expand(z_latent.shape[0], -1, -1)  # [n_samples, n_cells, 1]
