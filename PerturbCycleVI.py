...
class DecoderCCVI(nn.Module):
    ...
    def forward(
        ...
        # --- 1. Cell cycle angle output ---
        phase_vec = torch.tanh(self.phase_decoder(z))  # [N, 2] in [-1, 1]
        unit_vec = phase_vec / (phase_vec.norm(dim=1, keepdim=True) + 1e-8)

        cos_theta = unit_vec[:, 0].unsqueeze(1)  # [N, 1]
        sin_theta = unit_vec[:, 1].unsqueeze(1)  # [N, 1]

        # --- 2. Drug response ---
        r = self.drug_response_decoder(z, cont=d).squeeze(-1)  # [N]

        # --- 3. Compute eta = b + cycle + drug ---
        b_t = self.b_type[t]        # [N, G]
        theta = self.theta_type[t] # [N, G]

        # Cell cycle effect (broadcasted)
        w_cos_masked = self.w_cos * self.cycle_mask  # [G]
        w_sin_masked = self.w_sin * self.cycle_mask  # [G]
        cycle_effect = cos_theta * w_cos_masked + sin_theta * w_sin_masked  # [N, G]
        ...
        return (
            ...
            px_rate,
            None,
            unit_vec,       # now returns [cosθ, sinθ]
            r,
            None,
            torch.stack([self.w_cos, self.w_sin], dim=0),
            self.W_drug,
            self.b_type
        )
...
# 3. CONSTRUCTOR
    ...
    def __init__(
        ...
        from scvi.nn import Encoder  # Import Encoder  from scvi.nn
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        # Run decoder
        px_scale, theta, px_rate,_, alpha, r, _, W_cycle, W_drug, b_type = self.decoder(
...
adata = sc.read_h5ad("~/PerturbCycleVI/adata_Dinaciclib_balanced.h5ad")
...
# ─────────────────────────────────────────────────────────────
#  Usage (assuming adata is already loaded)
gene_list_path = os.path.expanduser("~/PerturbCycleVI/cell_cycle_genes.txt")
mask = create_cell_cycle_gene_mask(adata, gene_list_path)
...
model.save("~/PerturbCycleVI/ccvi_model_Dinaciclib_cluster", overwrite=True)
