...
# 5. Define setup_anndata for preproccessing AnnData
    ...
    def get_response(self, adata: AnnData | None = None, batch_size: int = 128) -> np.ndarray:
        """Returns the predicted drug response scalar r (≥0) for each cell."""
        adata = self._validate_anndata(adata)
        self.module.eval()
        r_all = []
    
        for tensors in self._make_data_loader(adata, batch_size=batch_size):
            z = self.module.z_encoder(tensors[REGISTRY_KEYS.X_KEY])[0]  # Mean of q(z|x)
            library = tensors[REGISTRY_KEYS.LIBRARY_SIZE_KEY]
            d = tensors.get(REGISTRY_KEYS.CONT_COVS_KEY)[:, 0:1] if REGISTRY_KEYS.CONT_COVS_KEY in tensors else None
            t = tensors[REGISTRY_KEYS.LABELS_KEY].squeeze(-1)
    
            with torch.no_grad():
                _, _, _, _, _, r, _, *_ = self.module.decoder(z, library, d, t)
            r_all.append(r.cpu().numpy())
    
        return np.concatenate(r_all)


    def get_theta(self, adata: AnnData | None = None, batch_size: int = 128) -> np.ndarray:
        """Returns the predicted cell cycle angle θ ∈ [0, 2π] for each cell."""
        adata = self._validate_anndata(adata)
        self.module.eval()
        theta_all = []
    
        for tensors in self._make_data_loader(adata, batch_size=batch_size):
            z = self.module.z_encoder(tensors[REGISTRY_KEYS.X_KEY])[0]
            library = tensors[REGISTRY_KEYS.LIBRARY_SIZE_KEY]
            d = tensors.get(REGISTRY_KEYS.CONT_COVS_KEY)[:, 0:1] if REGISTRY_KEYS.CONT_COVS_KEY in tensors else None
            t = tensors[REGISTRY_KEYS.LABELS_KEY].squeeze(-1)
    
            with torch.no_grad():
                _, _, _, _, _, _, theta, *_ = self.module.decoder(z, library, d, t)
            theta_all.append(theta.squeeze(1).cpu().numpy())
    
        return np.concatenate(theta_all)
