...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Baseline per drug concentration
        ...
        # Cell cycle effect via Fourier basis
        ...
        # First susceptibility branch (z[:, 1])
        self.susceptibility_net1 = nn.Sequential(
            nn.Linear(1, n_hidden),
            nn.ReLU(),
            nn.Linear(n_hidden, n_output)
        )

        # Second susceptibility branch (z[:, 2])
        self.susceptibility_net2 = nn.Sequential(
            nn.Linear(1, n_hidden),
            nn.ReLU(),
            nn.Linear(n_hidden, n_output)
        )
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        z_susc1 = z[:, 1].unsqueeze(1)  # [N, 1]
        z_susc2 = z[:, 2].unsqueeze(1)  # [N, 1]

        susceptibility_effect1 = self.susceptibility_net1(F.softplus(z_susc1))  # [N, G]
        susceptibility_effect2 = self.susceptibility_net2(F.softplus(z_susc2))  # [N, G]

        # Combine susceptibility effects additively
        susceptibility_effect = susceptibility_effect1 + susceptibility_effect2  # [N, G]
...
model = CCVI(adata,n_latent = 3,cycle_gene_mask=mask)
...
model.save("ccvi_model_dapagliflozin_2sus", overwrite=True)
