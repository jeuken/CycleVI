...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.drug_response_decoder = FCLayers(
            ...
            activation_fn=nn.ReLU,  # Standard ReLU hidden for drug response
            **kwargs
        )
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        # ➤ Use only control cells to compute mean and std
        raw_control = raw[control_mask]
        
        mean = raw_control.mean(dim=0, keepdim=True)
        std = raw_control.std(dim=0, keepdim=True) + 1e-6  # Avoid division by zero
        ...
        # Sigmoid and scale to [0, 2π]
        angle = 2 * torch.pi * torch.sigmoid(standardized)
        angle = angle.unsqueeze(1)  # shape: [N, 1]
        ...
        cycle_effect = (fourier_basis @ self.fourier_W) + self.offset  # [N, G]
...
model.save("ccvi_model_dapagliflozin_scale", overwrite=True)
