...
class Encoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.n_latent_cycle = 1
        self.n_latent_drug = n_latent - self.n_latent_cycle

        self.n_input = n_input
        self.register_buffer("cycle_gene_mask", cycle_gene_mask)
        self.n_input_cycle = int(cycle_gene_mask.sum()) if cycle_gene_mask is not None else n_input

        # Encoder for z0 (cycle)
        self.encoder_cycle = FCLayers(
            n_in=self.n_input_cycle,
            n_out=n_hidden,
        ...
        self.mean_cycle = nn.Linear(n_hidden, self.n_latent_cycle)
        self.var_cycle = nn.Linear(n_hidden, self.n_latent_cycle)

        # Encoder for z1 (drug)
        self.encoder_drug = FCLayers(
            n_in=self.n_input,
            n_out=n_hidden,
            n_cat_list=n_cat_list,
            n_layers=n_layers,
        ...
        self.mean_drug = nn.Linear(n_hidden, self.n_latent_drug)
        self.var_drug = nn.Linear(n_hidden, self.n_latent_drug)

    def forward(self, x_all: torch.Tensor, *cat_list: int):
        if self.cycle_gene_mask is not None:
            x_cycle = x_all[:, self.cycle_gene_mask]
        else:
            x_cycle = x_all

        # Encode z0 from cell cycle genes
        q_cycle = self.encoder_cycle(x_cycle, *cat_list)
        m0 = self.mean_cycle(q_cycle)
        v0 = self.var_activation(self.var_cycle(q_cycle)) + self.var_eps


        # Encode z1 from all genes
        q_drug = self.encoder_drug(x_all, *cat_list)
        m1 = self.mean_drug(q_drug)
        v1 = self.var_activation(self.var_drug(q_drug)) + self.var_eps

        # Sample and concatenate
        # Concatenate the distributions first
        m = torch.cat([m0, m1], dim=-1)
        v = torch.cat([v0, v1], dim=-1)
        dist = Normal(m, v.sqrt())
...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        n_output: int,
        n_cell_types: int,
        drug_concentrations: list[float] = [0.0, 0.05, 0.5, 5.0],
        n_layers: int = 1,
        n_hidden: int = 128,
        n_cat_list: Iterable[int] = None,
        ...
        **kwargs
        ...
        # Drug concentration mapping
        self.n_drug_concs = len(drug_concentrations)
        self.register_buffer("drug_concs_tensor", torch.tensor(drug_concentrations, dtype=torch.float32))

        # Baseline
        self.baseline_raw = nn.Parameter(0.01 * torch.randn(self.n_drug_concs, n_output))  # [D, G]

        # Dispersion
        self.disp_raw = nn.Parameter(0.01 * torch.randn(n_output))  # [G]

        # Cell cycle effect
        self.fourier_W = nn.Parameter(0.01 * torch.randn(2 * n_fourier, n_output))  # [B, G]
        ...
        # Susceptibility neural net: z1 → [N, G]
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        # Drug concentration → baseline
        d_diff = torch.abs(self.drug_concs_tensor.view(1, -1) - d)
        d_idx = torch.argmin(d_diff, dim=1).clamp(max=self.n_drug_concs - 1)  # [N]
        baseline = self.baseline_raw[d_idx]  # [N, G]

        # Cell cycle effect (z[:, 0])
        z_cycle = z[:, 0]
        standardized = (z_cycle - z_cycle.mean()) / (z_cycle.std() + 1e-6)
        angle = 2 * torch.pi * torch.sigmoid(standardized).unsqueeze(1)  # [N, 1]

        basis = [torch.cos(k * angle) for k in range(1, self.n_fourier + 1)] + \
                [torch.sin(k * angle) for k in range(1, self.n_fourier + 1)]
        fourier_basis = torch.cat(basis, dim=1)  # [N, B]

        cycle_effect = (fourier_basis @ self.fourier_W) * cycle_mask  # [N, G]

        # Susceptibility effect from z[:, 1] via MLP
        z_susc = z[:, 1].unsqueeze(1)  # [N, 1]
        susceptibility_effect = self.susceptibility_net(F.softplus(z_susc))  # [N, G]

        # Combine
        eta = baseline + cycle_effect + susceptibility_effect  # [N, G]
        px_scale = self.px_scale_activation(eta)
        px_rate = torch.exp(library) * px_scale
        disp = F.softplus(self.disp_raw)
...
# 7. Encodes input data into latent variables:
    def _regular_inference(
        ...
        """Run the regular inference process with library normalization and cell cycle mask support."""
        # --- Step 1: Compute observed library size (sum over genes per cell) ---
        library = torch.sum(x, dim=1, keepdim=True)  # [N, 1]
    
        # --- Step 2: Normalize expression per cell (counts per cell fraction) ---
        x_normalized = x / (library + 1e-8)
    
        # --- Step 3: Optionally log1p transform for stability ---
        ...
        # --- Step 5: Cell cycle gene subset ---
        x_cycle = x_normalized[:, self.cycle_gene_mask] if self.cycle_gene_mask is not None else x_normalized
        ...
        if self.batch_representation == "embedding" and self.encode_covariates:
            ...
            qz, z = self.z_encoder(encoder_input, x_cycle, *categorical_input)
        else:
            qz, z = self.z_encoder(encoder_input, x_cycle, *categorical_input)
    
        # --- Step 7: Optional library encoder if needed ---
        ql = None
        if not self.use_observed_lib_size:
            if self.batch_representation == "embedding":
        ...
        return {
            ...
            MODULE_KEYS.LIBRARY_KEY: torch.log(library + 1e-8),  # always log for decoder
        }
...
adata = sc.read_h5ad("adata_HT29_Dapagliflozin_balanced.h5ad")
...
model.save("ccvi_model_dapagliflozin_encoder", overwrite=True)
