...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Hill-type drug response
        self.hill_k = nn.Parameter(torch.full((1, n_output), 0.005))        # shape [1, G]
        self.hill_amplitude = nn.Parameter(0.01 * torch.randn(1, n_output))  # shape [1, G]

        # Global baseline and dispersion
        self.baseline_raw = nn.Parameter(0.01 * torch.randn(n_output))
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        basis_components = [torch.cos(k * angle) for k in range(1, self.n_fourier + 1)] + \
        ...
        # Hill drug effect
        k = torch.clamp(self.hill_k, min=1e-6, max=5.0)  # shape [1, G]
        sd = d                                           # shape [N, 1]
        hill_term = sd / (k + sd)                        # broadcasted to [N, G]
        drug_effect = hill_term * self.hill_amplitude  # shape [N, G]

        # Combine all terms
        eta = self.baseline_raw + cycle_effect + drug_effect  # [N, G]
        ...
        return (
            ...
            None,
            angle,        # [N, 1]
            None,            # [N, 1] (inferred susceptibility)
            None,
            self.fourier_W,
            self.hill_amplitude,
            self.baseline_raw
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        # Run decoder
        px_scale, disp, px_rate, _, angle, s, _, W_fourier, hill_amplitude, baseline = self.decoder(
...
model.save("ccvi_model_dapagliflozin_test", overwrite=True)
