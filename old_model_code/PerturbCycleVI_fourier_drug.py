...
class DecoderCCVI(nn.Module):
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        mean = raw_control.mean(dim=0, keepdim=True)
        std = raw_control.std(dim=0, keepdim=True) + 1e-6
        standardized = (raw - mean) / std
        angle = 2 * torch.pi * torch.sigmoid(standardized).unsqueeze(1)  # [N, 1]

        # --- Fourier basis and effect ---
        basis = torch.cat([
        ...
        cycle_effect = (basis @ self.fourier_W) + self.offset
        cycle_effect = torch.where(control_mask.unsqueeze(1), cycle_effect, cycle_effect.detach())
        cycle_effect = cycle_effect * cycle_mask

        # --- Drug response ---
        print("Treated cells in batch:", (~control_mask).sum().item())
        r = torch.zeros(z.size(0), 1, dtype=z.dtype, device=device)
        if (~control_mask).any():
        ...
        # --- Baseline and dispersion (control cells only contribute to gradients) ---
        b_t = self.b_type[t]
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        # Run decoder
        px_scale, disp, px_rate, _, angle, r, s, W_fourier, W_drug, b_type = self.decoder(
...
model.save("ccvi_model_dapagliflozin_uni", overwrite=True)
