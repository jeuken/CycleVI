...
class Encoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.scale_encoder = nn.Linear(n_hidden, 1)  # Only used for PowerSpherical
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        loc = self.mean_encoder(q)

        if self.distribution == "powerspherical":
            # Normalize loc vector
            loc_norm = loc.norm(dim=-1, keepdim=True)
            loc = torch.where(loc_norm < 1e-6, torch.tensor(1.0, device=loc.device), loc)
            loc = loc / (loc.norm(dim=-1, keepdim=True) + 1e-8)

            # Compute safe scale
            scale_logits = self.scale_encoder(q)
            raw_scale = self.var_activation(scale_logits)
            scale = torch.clamp(raw_scale.view(-1), min=1e-4)

            if torch.isnan(loc).any():
                raise ValueError("NaNs detected in normalized `loc` for PowerSpherical.")
            if torch.isnan(scale).any():
                raise ValueError("NaNs detected in computed `scale` for PowerSpherical.")
...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.k_max = 3  # ← new: number of harmonics
