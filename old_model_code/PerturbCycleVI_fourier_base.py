...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.W_drug = nn.Parameter(0.01 * torch.randn(1, n_output))
        self.b_type_all = nn.Parameter(torch.zeros(n_cell_types, n_output))
        self.disp_raw = nn.Parameter(torch.zeros(n_cell_types, n_output))
        self.px_scale_activation = nn.Softmax(dim=-1) if scale_activation == "softmax" else nn.Softplus()

        # One classifier per cell line dimension (z[2:])
        self.cell_line_classifier = nn.Sequential(
            nn.Linear(n_cell_types, 64),
            nn.ReLU(),
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        z_cycle = z[:, 0:1]
        z_identity = z[:, 1:2]
        z_baseline = z[:, 2:2 + self.n_cell_types]
        ...
        r = torch.zeros(z.size(0), 1, dtype=z.dtype, device=z.device)
        if (~control_mask).any():
            drug_input = torch.cat([z_cycle[~control_mask], z_identity[~control_mask]], dim=1)
            r_treated = F.softplus(self.drug_response_decoder(drug_input, cont=d[~control_mask]))
            r[~control_mask] = r_treated
        drug_effect = r @ self.W_drug

        # Predict soft cell line membership per z[2+i] via sigmoid classifier
        z_baseline_logits = self.cell_line_classifier(z_baseline)
        z_baseline_probs = torch.softmax(z_baseline_logits, dim=1)

        b_t = torch.matmul(z_baseline_probs, self.b_type_all)
        disp = F.softplus(torch.matmul(z_baseline_probs, self.disp_raw))
        ...
        return (
            ...
            disp,
            px_rate,
            z_baseline_logits,  # <- logits returned explicitly
            z_baseline_probs,  # <- softmax probabilities also returned
            angle,
            r,
            z_identity,
            self.fourier_W,
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        # Run decoder
        px_scale, disp, px_rate, z_baseline_logits, z_baseline_probs, angle, r, _, W_fourier, W_drug, b_type  = self.decoder(
        ...
        return {
            ...
            "angle": angle,
            "z_baseline_logits": z_baseline_logits,  # for loss
            "z_baseline_probs": z_baseline_probs     # for postanalysis
        }
    ...
    def loss(
        ...
        true_labels = tensors[REGISTRY_KEYS.LABELS_KEY].squeeze()
        ...
        kl_local = 0.1 * kl_weight * kl_divergence_z + kl_divergence_l
    
        # Classification loss using softmax outputs
        z_baseline_logits = generative_outputs["z_baseline_logits"]
        cls_loss = 1000 * F.cross_entropy(z_baseline_logits, true_labels)
    
        loss = torch.mean(reconst_loss + kl_local + cls_loss)
...
adata = sc.read_h5ad("adata_irinotecan_balanced.h5ad")
...
model.save("ccvi_model_irinotecan_base", overwrite=True)
