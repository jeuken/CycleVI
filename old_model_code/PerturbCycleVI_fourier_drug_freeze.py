...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # --- Sensitivity network (per cell, scalar, non-negative) ---
        self.sensitivity_nn = nn.Sequential(
        ...
        # --- Hill parameters ---
        self.hill_k = nn.Parameter(torch.tensor(0.5))
        ...
        self.unmask_cycle_at = kwargs.get("unmask_cycle_at", 200)
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        device = z.device
        if self.training:
            self.current_epoch.add_(1)
            if self.current_epoch == self.unmask_cycle_at:
                self.cycle_mask.data[:] = 1.0
        ...
        hill_val = d / (k + d)  # shape [N, 1]
        sensitivity = self.sensitivity_nn(z)           # shape [N, 1], non-negative
        drug_scalar = hill_val * sensitivity           # shape [N, 1]
        drug_effect = drug_scalar @ self.hill_amplitude  # shape [N, G]
        ...
        # L2 penalty on non-cycle Fourier weights
        noncycle_mask = 1.0 - self.cycle_mask.to(device)
        fourier_reg = torch.sum((self.fourier_W * noncycle_mask) ** 2)
...
# 7. Encodes input data into latent variables:
    ...
    def generative(
        ...
        px_scale, disp, px_rate, _, angle, drug_effect, _, W_fourier, fourier_reg, b_type = self.decoder(
