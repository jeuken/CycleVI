...
class Encoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.theta_encoder = nn.Linear(n_hidden, 1)  # angle for direction
        self.scale_encoder = nn.Linear(n_hidden, 1)  # concentration parameter
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        if torch.isnan(q).any():
            raise ValueError("NaNs detected in encoder output `q`.")

        if self.distribution == "powerspherical":
            # --- Compute angle θ and map to 2D unit vector
            theta = self.theta_encoder(q).squeeze(-1)  # shape: [N]
            ...
            if torch.isnan(loc).any():
                raise ValueError("NaNs detected in computed `loc`.")
            if torch.isnan(scale).any():
                raise ValueError("NaNs detected in computed `scale`.")

            dist = PowerSpherical(loc=loc, scale=scale)
            latent = dist.rsample()

            return (dist, latent) if self.return_dist else (loc, scale, latent)

        else:
            loc = self.theta_encoder(q)  # in non-circular case, treat θ as mean
            q_v = self.var_activation(self.scale_encoder(q)).squeeze(-1) + self.var_eps
            dist = Normal(loc, q_v.sqrt())
            latent = self.z_transformation(dist.rsample())

            return (dist, latent) if self.return_dist else (loc, q_v, latent)
...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Use full cycle modulation by default
        ...
        self.k_max = 2  # ← new: number of harmonics

        # Update W_cycle to accommodate 2 * k_max Fourier features
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        theta = torch.atan2(z[:, 1], z[:, 0])  # [N]

        # Create Fourier basis: [cos(kθ), sin(kθ)] for k = 1..k_max
        ...
        # Drug response
        r = torch.zeros(N, 1, dtype=z.dtype, device=device)
        if (~control_mask).any():
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        if self.latent_distribution == "powerspherical":
            pz = HypersphericalUniform(dim=z.shape[-1])
        else:
...
# 9. Computes total VAE loss
    def loss(
        ...
        kl_weight: torch.Tensor | float = 0.1,  # Weight to scale the KL divergence term
    ) -> LossOutput:
        """Compute the loss."""
        ...
        if isinstance(inference_outputs[MODULE_KEYS.QZ_KEY], PowerSpherical) and isinstance(generative_outputs[MODULE_KEYS.PZ_KEY], HypersphericalUniform):
            kl_divergence_z = kl_divergence( inference_outputs[MODULE_KEYS.QZ_KEY], generative_outputs[MODULE_KEYS.PZ_KEY]).sum(dim=-1)
        else:
            raise TypeError("KL divergence called with unsupported types:", type(inference_outputs[MODULE_KEYS.QZ_KEY]), type(generative_outputs[MODULE_KEYS.PZ_KEY]))
