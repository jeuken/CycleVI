...
class FourierFeatureMapping(nn.Module):
    def __init__(self, n_input, n_frequencies=4, scale=1.0):
        super().__init__()
        self.register_buffer(
            "frequencies", scale * torch.arange(1, n_frequencies + 1).float()
        )

    def forward(self, z):
        # z: [batch_size, n_input]
        out = []
        for freq in self.frequencies:
            out.append(torch.sin(freq * z))
            out.append(torch.cos(freq * z))
        return torch.cat(out, dim=-1)  # [batch_size, n_input * 2 * n_frequencies]
...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        n_fourier: int = 2,
        n_input_fourier: int = 4,     # New! Number of Fourier input frequencies
        **kwargs
    ):
        super().__init__()
        ...
        # --- Fourier Feature Expansion of input z ---
        self.fourier_mapper = FourierFeatureMapping(n_input, n_frequencies=n_input_fourier)

        # --- Angle prediction after Fourier mapping ---
        fourier_feature_dim = n_input * 2 * n_input_fourier  # expanded input dimension

        self.angle_hidden_decoder = FCLayers(
            n_in=fourier_feature_dim,
            n_out=n_hidden,
            n_cont=0,
            n_hidden=n_hidden,
            n_layers=1,
        ...
        self.angle_output = nn.Linear(n_hidden, 1)

        nn.init.xavier_uniform_(self.angle_output.weight)
        nn.init.normal_(self.angle_output.bias, mean=0.0, std=0.01)

        # --- Fourier weights and offset ---
        self.fourier_W = nn.Parameter(0.1 * torch.randn(2 * n_fourier, n_output))
        self.offset = nn.Parameter(0.1 * torch.randn(1, n_output))
        ...
        self.W_drug = nn.Parameter(0.1 * torch.randn(1, n_output))
        self.b_type = nn.Parameter(0.1 * torch.randn(n_cell_types, n_output))
        self.disp_raw = nn.Parameter(0.1 * torch.randn(n_cell_types, n_output))
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        # --- Fourier feature expansion of z ---
        z_fourier = self.fourier_mapper(z)

        # --- Predict angle ---
        h = self.angle_hidden_decoder(z_fourier)
        raw_angle = self.angle_output(h)
        angle = 2*torch.pi * torch.tanh(raw_angle)
        ...
        fourier_basis = torch.cat(basis_components, dim=1)  # [N, 2*n_fourier]
        ...
        if (~control_mask).any():
            ...
            raw_r = self.drug_response_decoder(z_drug, cont=d_drug)
...
model = CCVI(adata,n_latent = 10,cycle_gene_mask = mask)
