...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.register_buffer("non_cycle_mask", (~cycle_gene_mask).float())
        ...
        # Baseline
        self.baseline_raw = nn.Parameter(0.01 * torch.randn(self.n_drug_concs, n_output))
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        ...
        d_idx = torch.argmin(d_diff, dim=1).clamp(max=self.n_drug_concs - 1)
        baseline = self.baseline_raw[d_idx]
        ...
        fourier_basis = torch.cat(basis, dim=1)

        cycle_effect = fourier_basis @ self.fourier_W  # [N, G]

        # --- Stable constant L2 penalty directly on weights ---
        fourier_W_non_cycle = self.fourier_W[:, self.non_cycle_mask.bool()]  # [2B, G_non_cycle]
        #fourier_reg = (fourier_W_non_cycle ** 2).mean()  # scalar
        fourier_reg = torch.abs(fourier_W_non_cycle).mean()  # scalar
        # Susceptibility effect
        z_susc = z[:, 1].unsqueeze(1)
        susceptibility_effect = self.susceptibility_net(F.softplus(z_susc))

        eta = baseline + cycle_effect + susceptibility_effect
        ...
        return (
            ...
            None,
            self.baseline_raw,
            fourier_reg  # Now always > 0 unless weights go to zero
        )
...
# 7. Encodes input data into latent variables:
    ...
    def generative(
        ...
        # --- Run decoder ---
        px_scale, disp, px_rate, _, angle, s, _, W_fourier, hill_amplitude, baseline, fourier_reg = self.decoder(
        ...
        return {
            ...
            "angle": angle,
            "fourier_reg": fourier_reg,  # <-- MUST BE INCLUDED
        }
    ...
    def loss(
        ...
        # Add soft mask Fourier reg
        fourier_reg = generative_outputs.get("fourier_reg", 0.0)
        reg_weight = 10000
        reg_loss = reg_weight * fourier_reg
    
        loss = torch.mean(reconst_loss + weighted_kl_local + reg_loss)
...
model.save("ccvi_model_dapagliflozin_softmask", overwrite=True)
