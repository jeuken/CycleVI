    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.var_activation = var_activation if var_activation is not None else F.softplus
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        q_v = torch.clamp(q_v, min=self.var_eps, max=1e2)  # prevent tiny or huge variance

        if torch.isnan(q_m).any() or torch.isnan(q_v).any():
            raise ValueError("NaNs detected in encoder output: check input or initialization.")
...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Learnable gene-wise weights for cosθ and sinθ
        self.W_cos = nn.Parameter(torch.randn(n_output))  # [G]
        self.W_sin = nn.Parameter(torch.randn(n_output))  # [G]

        # Drug response and baseline
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor, t: torch.Tensor):
        assert d.ndim == 2 and d.size(1) == 1, "d must be shape [N, 1]"
        assert t.ndim == 1, "t must be shape [N]"

        control_mask = (d.squeeze() == 0)
        device = z.device
        cycle_mask = self.cycle_mask.to(device)

        # --- Cell cycle angle → unit vector ---
        phase_vec = torch.tanh(self.phase_decoder(z))           # [N, 2]
        norm = phase_vec.norm(dim=1, keepdim=True) + 1e-8
        unit_vec = phase_vec / norm                             # [N, 2]
        cos_theta, sin_theta = unit_vec[:, 0], unit_vec[:, 1]   # [N], [N]

        # cycle_effect = cosθ * W_cos + sinθ * W_sin
        w_cos_masked = self.W_cos * cycle_mask
        w_sin_masked = self.W_sin * cycle_mask
        cycle_effect = cos_theta.unsqueeze(1) * w_cos_masked + sin_theta.unsqueeze(1) * w_sin_masked  # [N, G]

        # --- Drug response ---
        raw_r = self.drug_response_decoder(z, cont=d).squeeze(-1)
        r = F.softplus(raw_r)
        ...
        b_t = self.b_type[t]
        disp = F.softplus(self.disp_raw[t])
        ...
        return (
            ...
            unit_vec,         # [cosθ, sinθ]
            r,
            None,             # placeholder for θ if needed
            torch.stack([self.W_cos, self.W_sin], dim=0),  # shape [2, G]
            self.W_drug,
            self.b_type
        )
...
# 8. Decodes latent z back to gene expression
    def generative(
        ...
        # Run decoder
        px_scale, disp, px_rate, _, phase_vec, r, _, W_cycle, W_drug, b_type = self.decoder(
