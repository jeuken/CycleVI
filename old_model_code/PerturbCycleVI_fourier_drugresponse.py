...
class DecoderCCVI(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # --- Baseline per gene ---
        self.baseline = nn.Parameter(torch.zeros(1, n_output))  # [1, G]
        ...
        # --- Hill parameters ---
        self.hill_k = nn.Parameter(torch.tensor(0.5))  # scalar > 0
        self.hill_n = nn.Parameter(torch.tensor(1.5))  # scalar > 0
        self.hill_amplitude = nn.Parameter(0.01 * torch.randn(1, n_output))  # per gene

        # --- Dispersion (shared across cells, per gene) ---
    ...
    def forward(self, z: torch.Tensor, library: torch.Tensor, d: torch.Tensor):
        assert z.shape[1] >= 2, "Latent space must be at least 2D"
        assert d.ndim == 2 and d.size(1) == 1

        device = z.device

        # --- Cell cycle effect from z[:, 0] ---
        raw = z[:, 0]
        standardized = (raw - raw.mean()) / (raw.std() + 1e-6)
        angle = 2 * torch.pi * torch.sigmoid(standardized).unsqueeze(1)
        #scaled = (raw - raw.min()) / (raw.max() - raw.min() + 1e-6)
        #angle = scaled * 2 * torch.pi

        basis_components = []
        for k in range(1, self.n_fourier + 1):
            basis_components.append(torch.cos(k * angle))
            basis_components.append(torch.sin(k * angle))
        fourier_basis = torch.cat(basis_components, dim=1)  # shape [N, 2n_fourier]

        cycle_effect = (fourier_basis @ self.fourier_W)

        # --- Susceptibility and Drug Effect from z[:, 1] ---
        s = F.softplus(z[:, 1]).unsqueeze(1)  # shape [N, 1]
        #sd = s * d  # shape [N, 1]
        sd = d
        k = torch.clamp(self.hill_k, min=1e-6)
        n = torch.clamp(self.hill_n, min=1e-3)

        sd_n = torch.pow(sd, n)
        hill_term = sd_n / (torch.pow(k, n) + sd_n)  # shape [N, 1]

        drug_effect = hill_term @ self.hill_amplitude  # shape [N, G]

        # --- Combine effects with baseline ---
        eta = self.baseline + cycle_effect + drug_effect  # shape [N, G]
        px_scale = self.px_scale_activation(eta)
        px_rate = torch.exp(library) * px_scale
        disp = F.softplus(self.disp_raw)  # shape [G]
...
# 7. Encodes input data into latent variables:
    def _regular_inference(
        ...
        x_cycle = x_[:, self.cycle_gene_mask] if self.cycle_gene_mask is not None else x_
...
model.save("ccvi_model_dapagliflozin_drug", overwrite=True)
